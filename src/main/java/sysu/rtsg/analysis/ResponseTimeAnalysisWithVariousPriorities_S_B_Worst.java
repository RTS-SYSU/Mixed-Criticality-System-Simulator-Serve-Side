...
public class ResponseTimeAnalysisWithVariousPriorities_S_B_Worst {
...
//        }
    ...
    private long FIFOPlocalBlocking(SporadicTask t, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, long[][] Ris, long Ri,
        ...
        ArrayList<HashSet<Resource>> LocalBlockingResources = FIFOPgetLocalBlockingResources_Dynamic(t, resources, tasks.get(t.partition));
        HashSet<Resource> shortLocalBlockingResource = LocalBlockingResources.get(0);
        HashSet<Resource> longLocalBlockingResource = LocalBlockingResources.get(1);
        ...
        Set<Integer> resource_required_set = new HashSet<>();

        for (int i = 0; i < tasks.get(t.partition).size(); i++) {
            SporadicTask hpTask = tasks.get(t.partition).get(i);
            if (hpTask.priority > t.priority) {
                resource_required_set.addAll(hpTask.resource_required_index);
            }
        }
        resource_required_set.addAll(t.resource_required_index);
...
//            if(resource_required_set.contains(res.id - 1)){
...
//            }
            ...
            for (int parition_index = 0; parition_index < res.partitions.size(); parition_index++)
            {
                ...
                if (partition != t.partition && 0 < norR) {
                    local_blocking += res.csl;
                    // t.fifop_arrivalblocking_overheads += (AnalysisUtils.FIFOP_LOCK + AnalysisUtils.FIFOP_UNLOCK);
                }
            }
...
//            local_blocking_each_resource.add(local_blocking);
    ...
    private ArrayList<HashSet<Resource>> FIFOPgetLocalBlockingResources_Dynamic(SporadicTask task, ArrayList<Resource> resources, ArrayList<SporadicTask> localTasks) {
        ...
        ArrayList<HashSet<Resource>> localBlockingResources = new ArrayList<>();
        HashSet<Resource> shortLocalBlockingResources = new HashSet<>();
        HashSet<Resource> longLocalBlockingResources = new HashSet<>();
        ...
        // short也有，long也有，那我就在long里面算就行了，short是仅short
        shortLocalBlockingResources.removeAll(localBlockingResources);
