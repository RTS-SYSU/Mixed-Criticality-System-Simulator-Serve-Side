...
//        }
    ...
    private long FIFOPResourceAccessTime(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, long[][] Ris, long time,
        ...
        ArrayList<ArrayList<Long>> requestsonRemoteP = new ArrayList<>();
        if(task.id == 21){
            System.out.println(1);
        }
        for (Resource res : resources) {
            ...
            requestsonRemoteP.add(new ArrayList<>());
            // 计算各资源产生的Direct/Indirect Spin Delay，将最新加入的'requestsLeftOnRemoteP中的list'传入，在函数内部更新；
            spin += getSpinDelayForOneResoruce(task, tasks, res, requestsLeftOnRemoteP.get(requestsLeftOnRemoteP.size() - 1), Ris, time, btbHit, useRi,requestsonRemoteP.get(requestsonRemoteP.size()-1));
        }
...
//        }
    ...
    private long getSpinDelayForOneResoruce(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, Resource resource,
                                            ArrayList<Long> requestsLeftOnRemoteP, long[][] Ris, long time, boolean btbHit, boolean useRi, ArrayList<Long> requestsOnRemoteP) {

        // 需要注意的前提是，此函数计算的是针对'单个'资源计算相应的访问&阻塞时间

        if(task.id == 21 && resource.id == 5){
            System.out.println(1);
        }
...
//                        requestsOnRemoteP.add(number_of_request_by_Remote_P);
//                    }
                    requestsOnRemoteP.add(number_of_request_by_Remote_P);
