...
public class DynamicMixedCriticalSystem {
    /* System Setup */
    ...
    public static ArrayList<ArrayList<EventTimePoint>> cpuEventTimePointRecords;
    ...
    public static void DynamicInitialize(ArrayList<BasicPCB> _totalTasks, HashMap<Integer, ArrayList<Integer>> _resourceRequiredPrioritiesArray, ArrayList<BasicResource> _totalResources,  int _totalCpuCoreNum,
                                      TreeMap<Integer, ArrayList<Integer>> _taskReleaseTimes, Boolean _isStartUpSwitch, Integer _criticalitySwitchTime)
    {
        ...
        for (int i = 0; i < _totalTasks.size(); ++i) {
            totalTasks.add(new ProcedureControlBlock(_totalTasks.get(i), _resourceRequiredPrioritiesArray.get(_totalTasks.get(i).staticTaskId)));
        }
    ...
    public static void SystemExecute()
    {
        ...
        for (ArrayList<ProcedureControlBlock> waitingTasks : waitingTasksPerCore)
        {
        ...
        for (ProcedureControlBlock runningTask : runningTaskPerCore)
        {
            ...
            if (runningTask.basicPCB.executedTime == runningTask.basicPCB.resourceAccessTime.get(runningTask.basicPCB.requestResourceTh))
            {
                ...
                if (accquireResource.basicResource.isOccupied)
                {
                    ...
                    // 任务新增：事件时间点
                    TaskEventTimePointsRecords.get(runningTask.basicPCB.dynamicTaskId).add(new EventTimePoint(runningTask.basicPCB.staticTaskId, runningTask.basicPCB.dynamicTaskId, "lock-attempt", systemClock, accquireResource.basicResource.id));
                    ...
                    cpuEventTimePointRecords.get(runningTask.basicPCB.baseRunningCpuCore).add(new EventTimePoint(runningTask.basicPCB.staticTaskId, runningTask.basicPCB.dynamicTaskId, "lock-attempt", systemClock, accquireResource.basicResource.id));

                    // 处理 indirect spinning 的情况：高优先级任务抢占低优先级的任务，然后高优先级的任务申请访问资源时进行自旋
                    ArrayList<ProcedureControlBlock> waitingTasks = waitingTasksPerCore.get(runningTask.basicPCB.baseRunningCpuCore);
                    for (ProcedureControlBlock waitingTask : waitingTasks)
                    {
                ...
                // runningTask获取资源，优先级提升到 resource-ceiling-priority / 动态协议提供的优先级 --> resourceRequiredPriority
                    ...
                    // 任务新增：事件时间点
                    TaskEventTimePointsRecords.get(runningTask.basicPCB.dynamicTaskId).add(new EventTimePoint(runningTask.basicPCB.staticTaskId, runningTask.basicPCB.dynamicTaskId, "locked", systemClock, accquireResource.basicResource.id));
                    ...
                    cpuEventTimePointRecords.get(runningTask.basicPCB.baseRunningCpuCore).add(new EventTimePoint(runningTask.basicPCB.staticTaskId, runningTask.basicPCB.dynamicTaskId, "locked", systemClock, accquireResource.basicResource.id));
    ...
    public static void ReleaseTasks()
    {
        ...
        if (itemTaskReleaseTimes != null && itemTaskReleaseTimes.getKey() == systemClock)
        {
            for (Integer releaseTaskId : itemTaskReleaseTimes.getValue())
            {
                ...
                TaskEventTimePointsRecords.get(releaseTask.basicPCB.dynamicTaskId).add(new EventTimePoint(releaseTask.basicPCB.staticTaskId, releaseTask.basicPCB.dynamicTaskId, "release", systemClock, -1));
    ...
    public static void IncreaseElapsedTime()
    {
        ...
        for (ProcedureControlBlock runningTask : runningTaskPerCore) {
            ...
            if (runningTask != null && !runningTask.basicPCB.isAccessLocalResource && !runningTask.basicPCB.isAccessGlobalResource && runningTask.basicPCB.executedTime == runningTask.basicPCB.totalNeededTime)
            {
                ...
                TaskEventTimePointsRecords.get(runningTask.basicPCB.dynamicTaskId).add(new EventTimePoint(runningTask.basicPCB.staticTaskId, runningTask.basicPCB.dynamicTaskId, "completion", systemClock, -1));

                // cpu 上的任务完成(状态发生变化)：normal-execution --> completion
                cpuEventTimePointRecords.get(runningTask.basicPCB.baseRunningCpuCore).add(new EventTimePoint(runningTask.basicPCB.staticTaskId, runningTask.basicPCB.dynamicTaskId, "completion", systemClock, -1));
        ...
        if (criticality_indicator == 1)
        {
            ...
            for (ArrayList<ProcedureControlBlock> waitingTasks : waitingTasksPerCore)
            {
                Iterator<ProcedureControlBlock> waitingTasksIterator = waitingTasks.iterator();
                while (waitingTasksIterator.hasNext())
                {
                    ProcedureControlBlock waitingTask = waitingTasksIterator.next();
    ...
    public static void ReleaseResource(ProcedureControlBlock runningTask) {
        ...
        TaskEventTimePointsRecords.get(runningTask.basicPCB.dynamicTaskId).add(new EventTimePoint(runningTask.basicPCB.staticTaskId, runningTask.basicPCB.dynamicTaskId, "unlocked", systemClock, releaseResource.basicResource.id));
        // cpu 上也需要记录释放资源的时间点
        cpuEventTimePointRecords.get(runningTask.basicPCB.baseRunningCpuCore).add(new EventTimePoint(runningTask.basicPCB.staticTaskId, runningTask.basicPCB.dynamicTaskId, "unlocked", systemClock, releaseResource.basicResource.id));
        ...
        if (!releaseResource.waitingQueue.isEmpty()) {
            ...
            ProcedureControlBlock waitingTask = releaseResource.waitingQueue.get(0);
            ...
            TaskEventTimePointsRecords.get(waitingTask.basicPCB.dynamicTaskId).add(new EventTimePoint(waitingTask.basicPCB.staticTaskId, waitingTask.basicPCB.dynamicTaskId, "locked", systemClock, releaseResource.basicResource.id));
            ...
            cpuEventTimePointRecords.get(waitingTask.basicPCB.baseRunningCpuCore).add(new EventTimePoint(waitingTask.basicPCB.staticTaskId, waitingTask.basicPCB.dynamicTaskId, "locked", systemClock, releaseResource.basicResource.id));

            // waitingTask 获取到资源之后，waitingTasks 中的阻塞任务状态就需要从 indirect-spinning --> blocked
            // arrival-blocking 还将继续保持直到任务释放资源
            for (ProcedureControlBlock _waitingTask : waitingTasksPerCore.get(waitingTask.basicPCB.baseRunningCpuCore))
            {
    ...
    public static void ChooseTaskToRun() {
        ...
        for (int i = 0; i < TOTAL_CPU_CORE_NUM; ++i) {
            ...
            if (waitingTask.basicPCB.priorities.peek() > runningTask.basicPCB.priorities.peek())
            {
                ...
                // 访问局部资源 / 自旋等待 均会导致某些任务遭受 arrival-blocking
                // 这里需要终结某些任务的arrival-blocking
                for (ProcedureControlBlock _waitingTask : waitingTasks) {
                    ArrayList<com.example.serveside.response.EventInformation> _taskInformations = TaskEventInformationRecords.get(_waitingTask.basicPCB.dynamicTaskId);
     ...
     * */
    public static void ChangeTaskState(ProcedureControlBlock task, String _state)
    {
     ...
     * @param _state 任务{@code task} 的新状态。
     * */
    public static void ChangeCpuTaskState(int runningCpuCore, ProcedureControlBlock task, String _state)
    {
