...
/**
 * {@code MrsPWorstCase} 用于计算系统任务的发布时间，使得指定任务在MrsP资源共享协议下达到最坏执行情况。
 * <p>
 *     {@code MrsPWorstCase} 使用贪心算法来最大化指定任务的调度和阻塞时延。一个任务在运行过程中受到的干扰有三种类型：到达阻塞（arrival-blocking）、高优先级任务的干扰以及访问资源时受到的阻塞。
 *     {@code MrsPWorstCase} 通过贪心算法分别最大化三种阻塞使得任务的响应时间最大，即任务达到最坏执行情况。
 * </p>
 * <p>
 *     <strong>注意:  为了使下面注释更具清晰度和详细性，记被查看最差运行情况的任务为sufferTask。</strong>
 * </p>
 */
public class DynamicWorstCase {
    /**
     * 系统任务信息。
     */
    ...
    /**
     * 在动态资源共享协议下考虑低优先级任务给 sufferTask 带来的 Arrival-Blocking。
     * <p>
     *     在动态资源共享协议下，低优先级任务持有局部资源、自旋等待或持有全局资源均可以造成 sufferTask 遭受到达阻塞。
     *     从上述两种情况中找到提供最长到达阻塞时长的任务并设计其发布时间。
     * </p>
     */
    public int DynamicConsiderArrivalBlocking(int sufferTaskId) {
        ...
        int maxArrivalBlockingResourceIndex = -1;
        ArrayList<Integer> resourceRequestPriorities = null;

        for (BasicPCB task : allocatedTaskSets.get(sufferTask.baseRunningCpuCore)) {
            resourceRequestPriorities = resourceRequiredPrioritiesArray.get(task.staticTaskId);
            // 低优先级任务 && 低优先级任务有在访问资源
            if (task.basePriority < sufferTask.basePriority && !task.accessResourceIndex.isEmpty()) {
                // 看看每个访问资源能够提供的最大阻塞时间
                for (int i = 0; i < task.accessResourceIndex.size(); ++i) {
                    BasicResource arrivalBlockingResource = totalResources.get(task.accessResourceIndex.get(i));

                    // 如果访问的资源是全局资源的话
                    if (arrivalBlockingResource.isGlobal) {
                        // 1. 如果自旋等待全局资源时优先级提升更高
                        if (resourceRequestPriorities.get(i) >= sufferTask.basePriority) {
                            // 该全局资源会被哪些 cpu 所使用以此来计算访问该全局资源可能导致的 Arrival-Blocking-Time
                            ArrayList<ArrayList<BasicPCB>> accessResourceSets = accessResourceTasks.get(arrivalBlockingResource.id);
                            int accessCpuNum = 0;
                            // 看看哪些 cpu 上的任务会访问 arrivalBlockingResource
                            for (int j = 0; j < accessResourceSets.size(); ++j) {
                                if (j != sufferTask.baseRunningCpuCore && !accessResourceSets.get(j).isEmpty()) {
                                    ++accessCpuNum;
                                }
                            }
                            ...
                            if (maxArrivalBlockingTime <= accessCpuNum * arrivalBlockingResource.c_low - 1) {
                                ...
                                maxArrivalBlockingResourceId = arrivalBlockingResource.id;
                                maxArrivalBlockingResourceIndex = i;
                            }
                        }
                        // 2. task 持有全局资源
                        else {
        ...
        if (maxArrivalBlockingTask != null) {
            resourceRequestPriorities = resourceRequiredPrioritiesArray.get(maxArrivalBlockingTask.staticTaskId);
        }
        ...
        if (maxArrivalBlockingTask != null && totalResources.get(maxArrivalBlockingResourceId).isGlobal) {
            // maxArrivalBlockingTask 自旋等待 maxArrivalBlockingResource 时可以阻塞 sufferTask
            if (resourceRequestPriorities.get(maxArrivalBlockingResourceIndex) >= sufferTask.basePriority) {
                // 按照第一次访问 blocking-resource 前所使用的 cpu 时长来进行安排任务的发布时间：时间越长越早发布
                // 每个 cpu 上选取出任务的条件是 : 与 sufferTask 的关联越少越好
                ArrayList<ArrayList<BasicPCB>> accessBlockingResourceTasks = accessResourceTasks.get(maxArrivalBlockingResourceId);
                // 挑选出每一个 cpu 上最适合访问 blocking-resource 的任务
                ArrayList<BasicPCB> suitableTasks = new ArrayList<>(SimpleSystemGenerator.total_partitions);
                // 记录每一个任务参与本次阻塞时是第几次发布
                ArrayList<Integer> suitableTasksReleaseIndex = new ArrayList<>(SimpleSystemGenerator.total_partitions);

                // suitableTasks 中每一个任务对 sufferTask 的阻塞时长
                ...
                for (int i = 0; i < accessBlockingResourceTasks.size(); ++i) {
                    ...
                    if (!accessBlockingResourceTasks.get(i).isEmpty() && i != sufferTask.baseRunningCpuCore) {
                        ...
                        // 不是 sufferTask 上的任务按照 cpu 顺序依次访问 blocking-resource
                ...
                for (BasicPCB tmpTask : suitableTasks) {
                    ...
                    // 设置并记录其带给 sufferTask 的阻塞时间以及其对应的发布时间
                ...
                // 设置 maxArrivalBlockingTask 的释放时间
            ...
            // maxArrivalBlockingTask 必须持有资源
                ...
                // 再处理一下 sufferTask 的发布时间
                releaseTime = baseReleaseTime + 1;
                recordTaskReleaseTimesHashMap.get(sufferTaskId).add(releaseTime);

                // 这里还是在 BlockingRecordArray 中记录一下，后面在计算 baseReleaseTime 会方便一点
                BlockingRecordArray.add(new BlockingRecord(maxArrivalBlockingTask.accessResourceIndex.indexOf(maxArrivalBlockingResourceId), maxArrivalBlockingResourceId, maxArrivalBlockingTask, 0,
                        null, null, null,
                        BlockingRecord.BlockingTypes.arrivalBlockingGlobalResource,
        ...
        // 如果是 local-task 进行阻塞的话，那么就相对简单：maxArrivalBlockingTask 从 system-clock=1 开始发布
        else if (maxArrivalBlockingTask != null && !totalResources.get(maxArrivalBlockingResourceId).isGlobal) {
            releaseTime = baseReleaseTime - maxArrivalBlockingTask.resourceAccessTime.get(maxArrivalBlockingTask.accessResourceIndex.indexOf(maxArrivalBlockingResourceId));
            recordTaskReleaseTimesHashMap.get(maxArrivalBlockingTask.staticTaskId).add(releaseTime);
        ...
        // 此时返回的 releaseTime 是 arrival-blocking 结束的时间点，也就是 high-priority-task 释放的时间点
    ...
    /**
     * 在 MrsP 协议下考虑高优先级任务给 sufferTask 带来的干扰。
     * <p>
     *     调整高优先级任务的发布时间使 sufferTask 受到所有高优先级任务的干扰（即所有高优先级任务在 sufferTask 发布之后，完成执行之前释放）。同时，最大化高优先级任务在访问全局资源时受到的自旋等待时间。
     * </p>
     */
    public int DynamicConsiderHighPriorityTasksInterference(int baseReleaseTime, int sufferTaskId) {
        ...
        // 首先看看前面 sufferTask 有没有被释放过，如果没有被释放过，那么我们就需要在 baseReleaseTime 的时候释放
            ...
            if (highPriorityTask.basePriority > sufferTask.basePriority) {
                ...
                if (!highPriorityTask.accessResourceIndex.isEmpty()) {
                    for (int i = 0; i < highPriorityTask.accessResourceIndex.size(); ++i) {
                        ...
                        BlockingRecordArray.add(new BlockingRecord(-1, -1, highPriorityTask, recordTaskReleaseTimesHashMap.get(highPriorityTask.staticTaskId).size() - 1,
                                null, null, null,
                        ...
                        if (accessResource.isGlobal) {
                            BlockingRecord lastBlockingRecord = DynamicConsiderAccessGlobalResourceSpinDelay(baseReleaseTime, highPriorityTask.staticTaskId, i, sufferTaskId, BlockingRecord.BlockingTypes.highPriorityTaskAccessGlobalResource, highPriorityTaskIsFirstBlockingRecord);
                ...
                if (!moreHigherPriorityTask.accessResourceIndex.isEmpty()) {
                    ...
                    for (int l = 0; l < moreHigherPriorityTask.accessResourceIndex.size(); ++l) {
                        ...
                        if (moreHigherTaskAccessResource.isGlobal) {
                            BlockingRecord lastBlockingRecord = DynamicConsiderAccessGlobalResourceSpinDelay(moreHigherPriorityTaskStartTime, moreHigherPriorityTask.staticTaskId, l, sufferTask.staticTaskId,
    ...
    /**
     * 在 MrsP 协议下，最大化 sufferTask 访问全局资源时最大化自旋等待时间。
     * @param baseReleaseTime sufferTask 开始使用 cpu 的时间点。
     * @param sufferTaskId 被查看最差运行情况的任务的静态标识符。
     */
    public int DynamicConsiderSufferTask(int baseReleaseTime, int sufferTaskId) {
        ...
        if (!sufferTask.accessResourceIndex.isEmpty()) {
            for (int i = 0; i < sufferTask.accessResourceIndex.size(); ++i) {
                ...
                if (accessResource.isGlobal) {
                    BlockingRecord lastBlockingRecord = DynamicConsiderAccessGlobalResourceSpinDelay(baseReleaseTime, sufferTask.staticTaskId, i, sufferTaskId, BlockingRecord.BlockingTypes.highPriorityTaskAccessGlobalResource,
    ...
    /**
     * 在 MrsP 协议下，{@code MrsPConsiderAccessGlobalResourceSpinDelay} 用于最大化高优先级任务或 {@code sufferTask} 访问某一全局资源时受到的自旋等待时间。
     * <p>
     ...
     */
    public BlockingRecord DynamicConsiderAccessGlobalResourceSpinDelay(int baseReleaseTime, int accessGlobalResourceTaskId, int accessResourceIndex, int sufferTaskId,
        ...
        for (int i = 0; i < suitableTasks.size(); ++i) {
            ...
            if (suitableTasksCoverIndex.get(i) != chooseTaskBlockingTimes.size()) {
                // 先调整一下发生冲突的任务的释放次序
                DynamicAdjustBaseReleaseTimeAndCurrentTaskTotalSpinDelay(suitableTasks.get(i).staticTaskId, suitableTasksCoverIndex.get(i), sufferTaskId);
            }
        }
        ...
        if (!suitableTasks.isEmpty()) {
            ...
            for (BasicPCB suitableTask : suitableTasks) {
                ...
                // 记录其带给 blockingTask 的阻塞时间以及其对应的发布时间
    ...
    /**
     * {@code DynamicAdjustBaseReleaseTimeAndCurrentTaskTotalSpinDelay} 用于去除 {@code immigrateTask} 的第 {@code coverReleaseIndex} 次发布，并重新计算 {@code BlockingRecordArray} 中每个 {@code BlockingRecord} 的起止时间。
     */
    public BlockingRecord DynamicAdjustBaseReleaseTimeAndCurrentTaskTotalSpinDelay(int immigrateTaskId, int coverReleaseIndex, int sufferTaskId) {
        // 开始设置阻塞任务的发布时间（设置为）
        int baseReleaseTime = 1;
        // 任务的发布时间
        int releaseTime = 0;
    ...
    /**
     * 在动态资源共享协议下，{@code DynamicGeneratedWorstCaseReleaseTime} 使用贪心算法为系统中的任务生成发布时间，使得 sufferTask 达到最差执行情况。
     * <p>
     *     {@code DynamicGeneratedWorstCaseReleaseTime} 按以下顺序计算系统中任务的发布时间，使 sufferTask 受到最大化的调度和阻塞时延。
     *     <ol>
     ...
     */
    public TreeMap<Integer, ArrayList<Integer>> DynamicGeneratedWorstCaseReleaseTime(ArrayList<BasicPCB> totalTasks, HashMap<Integer, ArrayList<Integer>> _resourceRequiredPrioritiesArray, ArrayList<BasicResource> totalResources, Integer sufferTaskId, Integer total_partitions)
    {
        this.resourceRequiredPrioritiesArray = _resourceRequiredPrioritiesArray;
        ...
        // allocatedTaskSets : 每一个 cpu 上有哪些任务在执行 / 按任务分配的处理器核心进行分组
        ...
        // 1. 考虑低优先级任务给 sufferTask 所带来的 arrival-blocking
        int baseReleaseTime = DynamicConsiderArrivalBlocking(sufferTaskId);

        // 2. 考虑高优先级任务给 sufferTask 所带的 arrival-blocking
        baseReleaseTime = DynamicConsiderHighPriorityTasksInterference(baseReleaseTime, sufferTaskId);

        // 3. 考虑 sufferTask 开始运行时访问 global-resource 时遭受的 spin-delay
        baseReleaseTime = DynamicConsiderSufferTask(baseReleaseTime, sufferTaskId);
