...
/**
 * {@code PWLPMixedCriticalSystem}用以模拟任务在PWLP (Preemptable Waiting Locking Protocol)资源共享协议下的执行并记录任务全周期执行情况。
 *
 * <p>
 * {@code PWLPMixedCriticalSystem} 主要由以下三个函数构成：
 * <ul>
 *   <li>{@link #PWLPInitialize(ArrayList, ArrayList, int, TreeMap, Boolean, Integer) PWLPInitialize}</li>
 *   <li>{@link #SystemExecute() SystemExecute}</li>
 *   <li>{@link #PackageTotalInformation() PackageTotalInformation}</li>
 * </ul>
 * 其余函数均用来辅助完成这三个函数的功能。
 * 函数{@code PWLPInitialize}初始化系统环境配置，例如任务信息、资源信息、任务发布时间等。
 * 函数{@code SystemExecute}模拟任务在PWLP资源共享协议下的执行。
 * 函数{@code PackageTotalInformation}打包整理任务在PWLP资源共享协议下的全周期执行情况并将相关信息传递给调用函数。
 * @author 陈炜琰
 * @author 李伊鹏
 * @author 许自泓
 * @author 介琛
 * */
public class PWLPMixedCriticalSystem {
     ...
     * 每种任务对应列表中的一项。
     */
    public static ArrayList<com.example.serveside.service.pwlp.ProcedureControlBlock> totalTasks;
     ...
     */
    public static ArrayList<com.example.serveside.service.pwlp.Resource> totalResources;
     ...
     */
    public static ArrayList<com.example.serveside.service.pwlp.ProcedureControlBlock> runningTaskPerCore;
     ...
     * */
    public static ArrayList<ArrayList<com.example.serveside.service.pwlp.ProcedureControlBlock>> waitingTasksPerCore;
    ...
    /**
     * 函数{@code PWLPInitialize()}初始化系统环境（例如任务信息、资源信息、任务发布时间等）、系统模拟运行过程使用到的属性（例如系统时钟、任务已经完成的次数、用于记录状态的列表等）。
     ...
     * */
    public static void PWLPInitialize(ArrayList<BasicPCB> _totalTasks,  ArrayList<BasicResource> _totalResources, int _totalCpuCoreNum,
                                         TreeMap<Integer, ArrayList<Integer>> _taskReleaseTimes, Boolean _isStartUpSwitch, Integer _criticalitySwitchTime)
    {
        ...
        for (BasicPCB totalTask : _totalTasks) {
            totalTasks.add(new ProcedureControlBlock(totalTask));
        }

        totalResources = new ArrayList<>(_totalResources.size());
        for (BasicResource resource : _totalResources)
            totalResources.add(new com.example.serveside.service.pwlp.Resource(resource));
    ...
    /**
     * 模拟器根据函数 {@code PWLPInitialize()} 初始化后的系统环境、任务信息、资源信息等来模拟任务的执行，并记录任务的执行情况。
    ...
    public static void SystemExecute()
    {
        ...
        for (ArrayList<com.example.serveside.service.pwlp.ProcedureControlBlock> waitingTasks : waitingTasksPerCore)
        {
            // 1. 终止 waiting Task 的状态
            for (com.example.serveside.service.pwlp.ProcedureControlBlock waitingTask : waitingTasks) {
     ...
     * <p>
     * 如果任务请求的资源正在被其他任务持有，那么任务加入到请求资源的FIFO队列末尾中，以自身优先级自旋等待资源，任务的执行状态变成直接自旋（direct-spinning）。
     * 除此之外，其余在该处理器上等待执行的任务其状态由阻塞（blocked）变成间接自旋（indirect-spinning）。
     * 注意，在PWLP协议下，自旋等待全局资源并不会提升任务的优先级，所以不会给其他任务带来 arrival-blocking。
     * </p>
     * <p>
    ...
    {
        for (com.example.serveside.service.pwlp.ProcedureControlBlock runningTask : runningTaskPerCore)
        {
            if (runningTask == null)
                continue;
            ...
            {
                com.example.serveside.service.pwlp.Resource accquireResource = totalResources.get(runningTask.basicPCB.accessResourceIndex.get(runningTask.basicPCB.requestResourceTh));
                ...
                if (accquireResource.basicResource.isOccupied)
                {
                    ...
                    // 这里的话 waitingTasks 上的任务都会受到 indirect-spinning（在 pwlp 协议下）
                    ArrayList<com.example.serveside.service.pwlp.ProcedureControlBlock> waitingTasks = waitingTasksPerCore.get(runningTask.basicPCB.baseRunningCpuCore);
                    for (com.example.serveside.service.pwlp.ProcedureControlBlock waitingTask : waitingTasks)
                    {
                ...
                // runningTask获取资源，优先级提升到 resource-ceiling-priority / 动态协议提供的优先级 --> resourceRequiredPriority
                    ...
                    // 访问局部资源的时候才需要优先级提升，对于全局资源则是直接不可抢占
                    if (!accquireResource.basicResource.isGlobal) {
                        runningTask.basicPCB.priorities.add(accquireResource.basicResource.ceiling.get(runningTask.basicPCB.baseRunningCpuCore));
                    }
    ...
    public static void IncreaseElapsedTime()
    {
        ...
        if (criticality_indicator == 1)
        {
            ...
            {
                com.example.serveside.service.pwlp.ProcedureControlBlock runningTask = runningTaskPerCore.get(i);
                ...
                if (runningTask.basicPCB.spin)
                {
                    ...
                    for (com.example.serveside.service.pwlp.ProcedureControlBlock _waitingTask : waitingTasksPerCore.get(i))
                    {
            ...
            for (ArrayList<com.example.serveside.service.pwlp.ProcedureControlBlock> waitingTasks : waitingTasksPerCore)
            {
                Iterator<com.example.serveside.service.pwlp.ProcedureControlBlock> waitingTasksIterator = waitingTasks.iterator();
                while (waitingTasksIterator.hasNext())
                {
                    com.example.serveside.service.pwlp.ProcedureControlBlock waitingTask = waitingTasksIterator.next();
     ...
     */
    public static void ReleaseResource(com.example.serveside.service.pwlp.ProcedureControlBlock runningTask) {
        // 要释放的资源
        com.example.serveside.service.pwlp.Resource releaseResource = totalResources.get(runningTask.basicPCB.accessResourceIndex.get(runningTask.basicPCB.requestResourceTh));
        ...
        // 只有 local-resource 获取优先级提升，所以释放资源时需要弹出优先级
        if (!releaseResource.basicResource.isGlobal) {
        ...
        if (!releaseResource.waitingQueue.isEmpty()) {
            // 说明这个一定是个 global-resource
            com.example.serveside.service.pwlp.ProcedureControlBlock waitingTask = releaseResource.waitingQueue.get(0);
            ...
            // pwlp 协议下 waitingTask 自旋等待资源时不会产生 arrival-blocking，因为等待资源不会有优先级提升并且是可以被抢占的 （waitingTask 此时是对应处理器上优先级最高的）
            for (com.example.serveside.service.pwlp.ProcedureControlBlock _waitingTask : waitingTasksPerCore.get(waitingTask.basicPCB.baseRunningCpuCore))
            {
                ArrayList<com.example.serveside.response.EventInformation> _taskInformations = TaskEventInformationRecords.get(_waitingTask.basicPCB.dynamicTaskId);
                if (!_taskInformations.isEmpty() && _taskInformations.get(_taskInformations.size() - 1).getState().equals("indirect-spinning"))
                {
     ...
     *         <li>当处理器核心正在执行任务并且任务正在访问全局资源时，执行的任务保持不变，因为该任务不可被抢占。</li>
     *         <li>
     *             当处理器核心正在执行任务（任务单纯执行或者任务访问局部资源）并且等待执行的任务列表中存在优先级更高的任务时，更高优先级的任务抢占正在执行的任务，占据处理器核心，被抢占的任务重新放入等待执行的列表中。
     *             (如果被抢占的任务正在自旋等待资源，那么取消其对资源的申请，等待下次重新被调度时再次重新申请资源，同时结束等待执行列表中部分任务的间接自旋状态（indirect-spinning）)
     *         </li>
     *     </ol>
     * </p>
     * <p>
    ...
    public static void ChooseTaskToRun() {
        ...
        for (int i = 0; i < TOTAL_CPU_CORE_NUM; ++i) {
            ...
            ArrayList<com.example.serveside.service.pwlp.ProcedureControlBlock> waitingTasks = waitingTasksPerCore.get(i);
            ...
            // PWLP协议下 runningTask 在访问全局资源时不能够被抢占
            ...
            if (waitingTask.basicPCB.priorities.peek() > runningTask.basicPCB.priorities.peek())
            {
                ...
                if (runningTask.basicPCB.spin) {
                    ...
                    // 该 cpu 上处在 indirect-spinning 状态的任务终结 indirect-spinning
                    for (com.example.serveside.service.pwlp.ProcedureControlBlock _waitingTask : waitingTasks) {
                        ArrayList<com.example.serveside.response.EventInformation> _taskInformations = TaskEventInformationRecords.get(_waitingTask.basicPCB.dynamicTaskId);
                        if (!_taskInformations.isEmpty() && _taskInformations.get(_taskInformations.size() - 1).getState().equals("indirect-spinning"))
                        {
                            // 由 indirect-spinning-delay 变成 blocked
        ...
        for (int i = 0; i < TOTAL_CPU_CORE_NUM; ++i)
        {
            ...
            // 3. runningTask 访问局部资源导致优先级激增
            // 4. runningTask 访问全局资源不可被抢占
            // spinning 不会导致 arrival-blocking，因为在PWLP协议下，任务请求资源会直接被撤销
            ArrayList<com.example.serveside.service.pwlp.ProcedureControlBlock> waitingTasks = waitingTasksPerCore.get(i);
            for (com.example.serveside.service.pwlp.ProcedureControlBlock waitingTask : waitingTasks)
            {
                // 判断 Arrival Blocking 发生的情况
        ...
        // 3. 处理 indirect-spinning 的情况（一个更低优先级的任务发布并且处理器上有高优先级任务正在自旋等待资源）
        for (int i = 0; i < TOTAL_CPU_CORE_NUM; ++i) {
            com.example.serveside.service.pwlp.ProcedureControlBlock runningTask = runningTaskPerCore.get(i);
            if (waitingTasksPerCore.get(i).isEmpty() || !runningTask.basicPCB.spin)
                continue;

            // indirect-spinning 发生的条件
            // 1. runningTask 正在自旋等待全局资源
            // 2. 优先级更低的任务（PWLP协议下其实就是所有在 waitingTasks 上的任务）
            ArrayList<com.example.serveside.service.pwlp.ProcedureControlBlock> waitingTasks = waitingTasksPerCore.get(i);
            for (com.example.serveside.service.pwlp.ProcedureControlBlock waitingTask : waitingTasks)
            {
                if (waitingTask.basicPCB.basePriority < runningTask.basicPCB.basePriority)
    ...
    public static com.example.serveside.response.ToTalInformation PackageTotalInformation()
    {
        ...
        for (com.example.serveside.response.TaskInformation releaseTaskInformation : releaseTaskInformations)
        {
            ...
            for (com.example.serveside.service.pwlp.ProcedureControlBlock staticTask : totalTasks)
     ...
     * */
    public static void ChangeTaskState(com.example.serveside.service.pwlp.ProcedureControlBlock task, String _state)
    {
     ...
     * @param _state 任务{@code task} 的新状态。
     * */
    public static void ChangeCpuTaskState(int runningCpuCore, com.example.serveside.service.pwlp.ProcedureControlBlock task, String _state)
    {
