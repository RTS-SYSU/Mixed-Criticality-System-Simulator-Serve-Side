...
public class MrsPWorstCase {
    ...
    * {@code AdjustAccessGlobalResourceBlockingRecord}调整 access-global-resource 类型的 BlockingRecord 中相关任务的发布时间和阻塞时长。
     ...
     * </p>
     */
    public int MrsPConsiderArrivalBlocking(int sufferTaskId) {
        ...
        for (BasicPCB task : allocatedTaskSets.get(sufferTask.baseRunningCpuCore)) {
            ...
            if (task.basePriority < sufferTask.basePriority && !task.accessResourceIndex.isEmpty()) {
                ...
                for (int arrivalBlockingResourceId : task.accessResourceIndex) {
                    ...
                    if (arrivalBlockingResource.ceiling.get(sufferTask.baseRunningCpuCore) >= sufferTask.basePriority) {
                        // 如果当前访问的资源是局部资源 / local resource
                        if (!arrivalBlockingResource.isGlobal && maxArrivalBlockingTime < arrivalBlockingResource.c_low - 1) {
                        ...
                        // 如果当前访问的资源是全局资源 / global resource
                        else if (arrivalBlockingResource.isGlobal) {
     ...
     * </p>
     */
    public int MrsPConsiderHighPriorityTasksInterference(int baseReleaseTime, int sufferTaskId) {
                ...
                if (!highPriorityTask.accessResourceIndex.isEmpty()) {
                    for (int i = 0; i < highPriorityTask.accessResourceIndex.size(); ++i) {
                        ...
                        if (accessResource.isGlobal) {
                            BlockingRecord lastBlockingRecord = MrsPConsiderAccessGlobalResourceSpinDelay(baseReleaseTime, highPriorityTask.staticTaskId, i, sufferTaskId, BlockingRecord.BlockingTypes.highPriorityTaskAccessGlobalResource, highPriorityTaskIsFirstBlockingRecord);
                ...
                if (!moreHigherPriorityTask.accessResourceIndex.isEmpty()) {
                    ...
                    for (int l = 0; l < moreHigherPriorityTask.accessResourceIndex.size(); ++l) {
                        ...
                        if (moreHigherTaskAccessResource.isGlobal) {
                            BlockingRecord lastBlockingRecord = MrsPConsiderAccessGlobalResourceSpinDelay(moreHigherPriorityTaskStartTime, moreHigherPriorityTask.staticTaskId, l, sufferTask.staticTaskId,
     ...
     */
    public int MrsPConsiderSufferTask(int baseReleaseTime, int sufferTaskId) {
        ...
        if (!sufferTask.accessResourceIndex.isEmpty()) {
            for (int i = 0; i < sufferTask.accessResourceIndex.size(); ++i) {
                ...
                if (accessResource.isGlobal) {
                    BlockingRecord lastBlockingRecord = MrsPConsiderAccessGlobalResourceSpinDelay(baseReleaseTime, sufferTask.staticTaskId, i, sufferTaskId, BlockingRecord.BlockingTypes.highPriorityTaskAccessGlobalResource,
     ...
     */
    public BlockingRecord MrsPConsiderAccessGlobalResourceSpinDelay(int baseReleaseTime, int accessGlobalResourceTaskId, int accessResourceIndex, int sufferTaskId,
        ...
        for (int i = 0; i < suitableTasks.size(); ++i) {
            ...
            if (suitableTasksCoverIndex.get(i) != chooseTaskBlockingTimes.size()) {
                // 先调整一下发生冲突的任务的释放次序
                MrsPAdjustBaseReleaseTimeAndCurrentTaskTotalSpinDelay(suitableTasks.get(i).staticTaskId, suitableTasksCoverIndex.get(i), sufferTaskId);
            }
        }
    ...
    /**
     * {@code MrsPAdjustBaseReleaseTimeAndCurrentTaskTotalSpinDelay} 用于去除 {@code immigrateTask} 的第 {@code coverReleaseIndex} 次发布，并重新计算 {@code BlockingRecordArray} 中每个 {@code BlockingRecord} 的起止时间。
     */
    public BlockingRecord MrsPAdjustBaseReleaseTimeAndCurrentTaskTotalSpinDelay(int immigrateTaskId, int coverReleaseIndex, int sufferTaskId) {
        // 开始设置阻塞任务的发布时间（设置为）
        int baseReleaseTime = 1;
        // 任务的发布时间
        int releaseTime = 0;
     ...
     */
    public TreeMap<Integer, ArrayList<Integer>> MrsPGeneratedWorstCaseReleaseTime(ArrayList<BasicPCB> totalTasks, ArrayList<BasicResource> totalResources, Integer sufferTaskId, Integer total_partitions)
    {
        ...
        int baseReleaseTime = MrsPConsiderArrivalBlocking(sufferTaskId);

        // 2. 考虑高优先级任务给 sufferTask 所带的 arrival-blocking
        baseReleaseTime = MrsPConsiderHighPriorityTasksInterference(baseReleaseTime, sufferTaskId);

        // 3. 考虑 sufferTask 开始运行时访问 global-resource 时遭受的 spin-delay
        baseReleaseTime = MrsPConsiderSufferTask(baseReleaseTime, sufferTaskId);
